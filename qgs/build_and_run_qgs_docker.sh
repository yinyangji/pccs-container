#!/bin/bash
#
# 构建 QGS 镜像并启动容器（与 aesm-service/build_and_run_aesm_docker.sh 类似：build + run 一体）。
# 运行参数见本目录 README.md（1.1 Start QGS Service）。
#
...
IMAGE_NAME="${IMAGE_NAME:-localhost/local/qgs-container:ubuntu24.04}"
CONTAINER_NAME="${CONTAINER_NAME:-qgs}"
...
usage: $(basename "$0") [OPTION]...
  ...
  -t <tag>                          仅改 tag，镜像名为 localhost/local/qgs-container:<tag>
  -i <repo:tag>                     完整镜像名，优先级高于 -t 与环境变量 IMAGE_NAME
  -f                                docker build --no-cache
  -h                                帮助

环境变量：IMAGE_NAME、CONTAINER_NAME
EOM
    exit 1
}
...
process_args() {
    ...
    if [[ -n "${explicit_image}" ]]; then
        ...
        IMAGE_NAME="localhost/local/qgs-container:${tag}"
    fi
}
...
# README.md 1.1 Start QGS Service
run_container() {
