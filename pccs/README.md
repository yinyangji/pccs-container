...
本目录下 `./build_and_run_pccs_docker.sh` 默认**先构建镜像再启动容器**（与仓库 `aesm-service/build_and_run_aesm_docker.sh` 用法类似）。请先启动 AESM 容器，使 **`aesm.socket` 出现在与 PCCS 相同的宿主机目录**（默认 **`~/aesmd-shared`**，环境变量 **`AESMD_SOCKET_DIR`**），再运行 PCCS。仅构建：`-a build`；仅启动已有镜像：`-a run`。
